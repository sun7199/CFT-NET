...
class PLModel(pl.LightningModule):
    ...
    def compute_input_saliency(self,batch, mode="similarity"):
        """
        返回与输入同形状的显著图: (B, 63, 250)
        不更改参数 .grad，适合在 val/test 中调用。
        """
        # Lightning 已把 batch 移到设备；确保可追踪
        with torch.enable_grad():
            eeg = batch['eeg']                          # (B,63,250)
            eeg = eeg.detach().requires_grad_(True)

            eeg_z,_ = self.brain(eeg)                     # (B, D)
            if mode == "similarity":
                # 与配对图像特征的相似度作为得分（示例）
                img_z = batch['img_features']           # (B, D)
                eeg_z = F.normalize(eeg_z, dim=-1)
                img_z = F.normalize(img_z, dim=-1)
                score = (eeg_z * img_z).sum(dim=-1).sum()  # 标量
            elif mode == "sum":
                score = eeg_z.sum()                     # 简单可导标量
            else:
                raise ValueError("unknown mode")

            # 仅对输入求梯度，不污染参数 .grad
            grads = torch.autograd.grad(
                outputs=score, inputs=eeg,
                retain_graph=False, create_graph=False, allow_unused=False
            )[0]                                        # (B,63,250)

            # Input×Grad（更稳定），并归一化到 [0,1]
            sal = (grads * eeg).abs()                         # Input×Grad
            denom = sal.amax(dim=(1, 2), keepdim=True) #在维度 1 和 2（通道、时间）上取最大值；
            sal = sal / (denom + 1e-9)
            return sal.detach()
    ...
    def training_step(self, batch, batch_idx):
        ...
        # self.eeg_outputs.append(eeg1.detach().cpu()) 
    ...
    #     self.img_outputs.clear()

    def validation_step(self, batch, batch_idx):
        # sal = self.compute_input_saliency(batch)  # (B,63,250)
        # npy = sal.cpu().numpy()
        # np.save("saliency_val.npy",npy)
        batch_size = batch['idx'].shape[0]
    ...
    def test_step(self,batch, batch_idx):
        batch_size = batch['idx'].shape[0]
        input_eeg=batch["eeg"].detach().cpu()
        np.save("raw_eeg",input_eeg)
        eeg_z, img_z, loss = self(batch)
        ...
        top_kvalues, top_k_indices = similarity.topk(5, dim=-1)
        self.all_predicted_classes.append(top_k_indices.cpu().numpy())
        label = torch.arange(200, device="cuda:0")
        self.all_true_labels.extend(label.cpu().numpy())
        return loss
...
def main():
    ...
    trainer = Trainer(log_every_n_steps=10, callbacks=[checkpoint_callback],max_epochs=config['train']['epoch'], devices=[device],accelerator='cuda',logger=logger)
