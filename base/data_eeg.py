...
def load_eeg_data(config):
    ...
    if exp_setting == 'intra-subject':
        ...
        train_loader = DataLoader(train_dataset, batch_size=config['data']['train_batch_size'], shuffle=False, drop_last=False, num_workers=32, pin_memory=True)
...
class EEGDataset(Dataset):
    def __init__(self, config, mode):
        ...
        if self.config['data']['uncertainty_aware']:
            ...
            data_dir = os.path.join(self.data_dir,'../Image_feature/CLIP')
    ...
    def ImageEncoder(self,images,blur_transform=None):
        ...
        image_features_list = torch.from_numpy(np.load("/home/yueming/github/NICE-EEG/dnn_feature/clip_feature_maps_training.npy")).float().to(device)
